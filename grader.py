"""
grader.py — Deterministic task graders for Digital Behavior Coach
All tasks are seeded with random.seed(42) for reproducible scores.
...
def run_task(task_name: str, scenario: dict, actions: list, success_fn) -> float:
    """Run a fixed action sequence against a given scenario and return a score."""
    random.seed(42)  # re-seed before every task for full reproducibility
    env = DigitalCoachEnv()
    env.reset()
...
def task_easy() -> float:
    """Easy: Agent should reduce screen time for a user exceeding 5 hours."""
    scenario = {
        "hour_of_day": 15,
    ...
    def success(total_reward, steps, final_state):
        return min(1.0, max(0.0, total_reward / (steps * 0.6)))

    return run_task("Easy: Reminder Task", scenario, actions, success)


def task_medium() -> float:
    """Medium: Help a sleep-goal user disengage late at night."""
    scenario = {
        "hour_of_day": 22,
    ...
    def success(total_reward, steps, final_state):
        return min(1.0, max(0.0, total_reward / (steps * 0.65)))

    return run_task("Medium: Sleep Intervention", scenario, actions, success)


def task_hard() -> float:
    """Hard: Maintain healthy habits across a full day without over-intervening."""
    scenario = {
        "hour_of_day": 9,
    ...
    def success(total_reward, steps, final_state):
        ...
        return min(1.0, max(0.0, (total_reward / (steps * 0.55)) + streak_bonus))
