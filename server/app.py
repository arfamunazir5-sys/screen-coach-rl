...
def _build_coach_md(action_type, reward_val, reason, done, step):
...
def _build_history_md():
    ...
    for h in reversed(_history[-15:]):
        ...
        rows.append(f"| {h['step']} | {em} `{h['action']}` | `{h['reward']:.2f}` | `{h['screen_time']:.1f}h` | `{h['habit_score']:.2f}` | `{h['resistance']:.2f}` |")
    return "\n".join(rows)
...
def _build_chart():
    ...
    if not _history:
        fig = go.Figure()
        fig.add_annotation(text="Take an action to see live performance stats",
    ...
    fig.add_trace(go.Scatter(x=steps, y=rewards, name="Reward",
        ...
        fillcolor="rgba(16,185,129,0.07)", mode="lines+markers", marker=dict(size=5)))
    fig.add_trace(go.Scatter(x=steps, y=[s/12 for s in screens], name="Screen (÷12)",
    ...
    fig.add_trace(go.Scatter(x=steps, y=habits, name="Habit Score",
        line=dict(color="#38bdf8", width=2), mode="lines+markers", marker=dict(size=5)))
    fig.add_trace(go.Scatter(x=steps, y=resists, name="Resistance",
        line=dict(color="#f59e0b", width=1.5, dash="dot"), mode="lines"))
    fig.update_layout(paper_bgcolor=dark, plot_bgcolor=panel,
        ...
        margin=dict(l=50, r=20, t=40, b=40), height=320, hovermode="x unified")
    return fig
...
def reset_ui():
    ...
    _history = []
    obs = _ui_env.reset()
    return (_build_state_md(obs), _build_chart(),
...
def take_action(action_type):
    ...
    _history.append({"step": info["step"], "action": action_type,
...
body, .gradio-container { background: #020817 !important; font-family: 'JetBrains Mono', monospace !important; }
.gradio-container { max-width: 1100px !important; margin: 0 auto !important; }
.gr-markdown h1 { font-family: 'Syne', sans-serif !important; font-size: 1.9rem !important; font-weight: 800 !important; background: linear-gradient(135deg, #38bdf8 0%, #818cf8 50%, #34d399 100%) !important; -webkit-background-clip: text !important; -webkit-text-fill-color: transparent !important; }
.gr-markdown h2 { font-family: 'Syne', sans-serif !important; font-size: 0.95rem !important; color: #38bdf8 !important; text-transform: uppercase !important; letter-spacing: 0.08em !important; }
.gr-markdown code { background: #0d2847 !important; border: 1px solid #1e3a5f !important; color: #34d399 !important; padding: 0.08rem 0.4rem !important; border-radius: 5px !important; }
.gr-markdown blockquote { border-left: 3px solid #818cf8 !important; background: #0d1526 !important; padding: 0.55rem 1rem !important; border-radius: 0 8px 8px 0 !important; color: #c4b5fd !important; font-style: italic !important; margin: 0.6rem 0 !important; }
.gr-markdown table { border-collapse: collapse !important; width: 100% !important; font-size: 0.8rem !important; }
.gr-markdown th { background: #0d1f38 !important; color: #38bdf8 !important; padding: 0.4rem 0.65rem !important; border: 1px solid #1e3a5f !important; text-transform: uppercase !important; font-size: 0.7rem !important; }
.gr-markdown td { padding: 0.38rem 0.65rem !important; border: 1px solid #0f2744 !important; color: #94a3b8 !important; }
button.lg { background: #0d1526 !important; border: 1px solid #1e3a5f !important; color: #94a3b8 !important; border-radius: 10px !important; font-family: 'JetBrains Mono', monospace !important; font-weight: 600 !important; transition: all 0.2s ease !important; }
button.lg:hover { transform: translateY(-2px) !important; box-shadow: 0 0 20px rgba(56,189,248,0.2) !important; border-color: #38bdf8 !important; color: #e2e8f0 !important; }
button.primary { background: linear-gradient(135deg, #1e3a5f, #0d2847) !important; border: 1px solid #38bdf8 !important; color: #38bdf8 !important; }
"""
...
with gr.Blocks(theme=gr.themes.Base(primary_hue="blue", neutral_hue="slate"),
    ...
    with gr.Row():
        ...
        gr.Markdown("<div style='padding:0.6rem 0;color:#334155;font-size:0.78rem;'>Reset starts a new episode. Take actions to coach the user.</div>")
    with gr.Row():
    ...
    gr.Markdown("<div style='text-align:center;color:#1e3a5f;font-size:0.72rem;margin-top:1rem;border-top:1px solid #0f2744;padding-top:0.6rem;'>screen-coach-rl · OpenEnv v1.0 · Tasks: easy / medium / hard</div>")
